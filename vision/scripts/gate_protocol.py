...
class Gate_Protocol():
    ...
    def spin(self, obstacle_name):
        ...
        while self.detectionArray == None:
            current_time = time.time()
            if current_time - starting_time > 30:
                return False
            print("Cannot find Gate")
        OBS_X = self.detectionArray.boundingBox.center.x # pixel vs normalized 0 - 1
    ...
    def shimmy(self):
        ...
        # controls --> goto self.set_points[0] # going back to the starting position
    ...
    def orbit(self):
        ...
        # +++ controls: go to *self.set_points[position_around_circle%self.NUM_SET_POINTS]

        # current_gate_width = self.gate_width
        # starting_time = time.time()
        # while current_gate_width == None:
        #     current_gate_width = self.gate_width
        #     current_time = time.time()
        #     if current_time - starting_time > 5:
        #         # +++ controls: go to *self.set_points[(position_around_circle+1)%self.NUM_SET_POINTS]
        #         position_around_circle += 1
        #         break
